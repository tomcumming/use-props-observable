...
export default function reactiveComponent<Props extends object>(
    render: (props$: Observable<Props>) => Observable<Rendered>,
    compare: (a: Props, b: Props) => boolean = shallowCompare
): React.StatelessComponent<Props> {
    return function ReactiveComponent(props: Props): Rendered {
        const ref = React.useRef<State<Props>>({
            subj: null as any, // avoid creating this every render
            last: null,
            invalidatedCount: 0,
            insideRender: false
        });
        if(ref.current.subj === null)
            ref.current.subj = new BehaviorSubject<Props>(props);
